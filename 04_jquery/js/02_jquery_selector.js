...
$('#btn3').on('click', ()=>{
    // input[type=text]
    $(":text")
        .css('background-color', 'tomato') // 원래 jquery 객체를 다시 리턴. 메소드 chaining
        .css('color', 'white');
    // 객체형을 전달가능 (css 속성명 주의 : -이 있다면 문자열 처리 또는 카멜케이싱)
    $(":password")
        .css({
    ...
    $(":button").val('Click me!'); // input타입 버튼만
});
...
$(btn4).on('click', () => {
    ...
    // const $selected = $("#people tr:contains('A')"); // 텍스트 기반
    const $selected = $("#people tr:has(a[href])"); // 텍스트 기반
