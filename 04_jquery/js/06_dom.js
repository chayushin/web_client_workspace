...
$(btn1).click(()=> {
    ...
    // const $h3 = $('<h3>Helloworld</h3>'); // 메모리상의 태그객체
    const $h3 = $('<h3></h3>');
    $h3.append('안녕');
    $area.html($h3);
});
 ...
 */
$(btn6).click(() => 
    $(`<span>${String.fromCharCode(char++)}</span>`).appendTo($target).css('color', 'hotpink'));
$(btn7).click(() => 
    $(`<span>${String.fromCharCode(char++)}</span>`).prependTo($target).css('color', 'hotpink'));
$(btn8).click(() => 
    $(`<span>${String.fromCharCode(char++)}</span>`).insertAfter($target).css('color', 'hotpink'));
$(btn9).click(() => 
    $(`<span>${String.fromCharCode(char++)}</span>`).insertBefore($target).css('color', 'hotpink'));
