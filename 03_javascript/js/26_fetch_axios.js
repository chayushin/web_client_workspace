 ...
 *  - {"id":"shqkel","company":"KH정보교육원","classroom":"352","cnt":108}
 *  - 속성명은 반드시 쌍따옴표로 감싼다.
 *  - 문자열은 반드시 쌍따옴표로 감싼다.
...
document.querySelector('#btn1').addEventListener('click', () => {
    ...
    fetch(url)
        .then((response) => {
            // fetch의 첫번째 콜백함수는 응답이 시작되는 순간 호출 -> 응답테이터 확인불가
            console.log(response);
            return response.json(); // 응답테이터중 json 추출
        })
        // .then((text) => {
...
document.querySelector('#btn2').addEventListener('click', () => {
    const url = 'https://api.github.com/users/chayushin';
    fetch(url)
...
/**
 * axios
 * - 내부적으로 XMLHttpRequest객체를 promise 기반으로 사용하도록 하는 js라이브러리
 ...
 * 
 * - live_ZrzWREGGJcfNisKfsjFHxoONjKJZdF5Qj2DVIDgyL6BelhqZCvGgoVnG5Ax7P8aU
 */
document.querySelector('#btn5').addEventListener('click', () => {
    ...
    axios(url, {
        headers :{
            'x-api-key' : 'live_ZrzWREGGJcfNisKfsjFHxoONjKJZdF5Qj2DVIDgyL6BelhqZCvGgoVnG5Ax7P8aU'
        },
        params : {
            limit : 1, // 기본값
            size : 'full'
