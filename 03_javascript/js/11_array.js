...
function test4() {
    ...
    // buttons.map(function(n) {console.log(n);}); // Uncaught TypeError: buttons.map is not a function at test4
...
function test6() {
    ...
    console.log(alpha.splice(1, 1, 'x', 'y')); // ['b'] 반환, alpha를 ['a', 'x', 'y', 'c', 'd', 'e']로 변경
    console.log(alpha.splice(1, 0, 'k', 'l', 'm')); // []반환,  ['a', 'k', 'l', 'm', 'x', 'y', 'c', 'd', 'e']
...
function test9() {
    ...
    // 1. button배열 생성 (진짜배열로 반환)
    // 2. map함수를 이용해서 button태그안의 innerHTML속성값을 배열에 담아 반환

    // const buttons = document.getElementsByTagName("button");
    // const buttonsAry = Array.from(buttons);
    // const buttonsIH = buttonsAry.map(function(n){
    //     return n.innerHTML;
    // });
    // console.log(buttonsIH);
...
function test10() {
    ...
    // const buttonsIT = 
    // [...document.querySelectorAll("button")].reduce(function(agg, button){
    //     agg.push(button.innerText);
    //     return agg;
    // }, []);
    // console.log(buttonsIT);
